// Copyright 2014 The auc Authors
...
func (srv *Server) NodeInfo() *NodeInfo {
	...
	x8 := node.X8()
	xy := node.XY()
	mid := "0x" + common.Bytes2Hex(x8[:])
	pubkey := common.Bytes2Hex(xy[:])
	info := &NodeInfo{
		Name:       srv.Name,
		Enode:      node.String(),
		MID:        mid,
