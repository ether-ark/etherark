...
func TestUpdateLeaks(t *testing.T) {
	...
	for i := byte(0); i < 255; i++ {
		...
		state.AddBalance(addr, big.NewInt(int64(11*i)), big.NewInt(1))
...
func TestIntermediateLeaks(t *testing.T) {
	...
	modify := func(state *StateDB, addr common.Address, i, tweak byte) {
		state.SetBalance(addr, big.NewInt(int64(11*i)+int64(tweak)), big.NewInt(1))
...
// https://github.com/ether-ark/etherark/pull/15549.
func TestCopy(t *testing.T) {
	...
	for i := byte(0); i < 255; i++ {
		...
		obj.AddBalance(big.NewInt(int64(i)), big.NewInt(1))
	...
	for i := byte(0); i < 255; i++ {
		...
		origObj.AddBalance(big.NewInt(2 * int64(i)), big.NewInt(1))
		copyObj.AddBalance(big.NewInt(3 * int64(i)), big.NewInt(1))
	...
	actions := []testAction{
		{
			...
			fn: func(a testAction, s *StateDB) {
				s.SetBalance(addr, big.NewInt(a.args[0]), big.NewInt(1))
			},
			args: make([]int64, 1),
		},
		{
			name: "AddBalance",
			fn: func(a testAction, s *StateDB) {
				s.AddBalance(addr, big.NewInt(a.args[0]), big.NewInt(1))
			},
			args: make([]int64, 1),
		},
		{
...
func (s *StateSuite) TestTouchDelete(c *check.C) {
	...
	s.state.AddBalance(common.Address{}, new(big.Int), big.NewInt(1))
...
// See https://github.com/ether-ark/etherark/pull/15225#issuecomment-380191512
func TestCopyOfCopy(t *testing.T) {
	sdb, _ := New(common.Hash{}, NewDatabase(ethdb.NewMemDatabase()))
	addr := common.HexToAddress("aaaa")
	sdb.SetBalance(addr, big.NewInt(42), big.NewInt(1))
